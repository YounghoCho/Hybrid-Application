...
$insertq="update tb_board5 set view=view+1 where b_num='".$_GET['b_num']."' ";
$insertr= sql_query($insertq);
$insertb="update tb_board5 set best=best+1 where b_num='".$_GET['b_num']."' ";
...
<!-------------------------------------------->	
<?
$sql = "select * from tb_board5 where b_idx = '".$_GET["b_num"]."' ";
...
<div style="background-color:#fff">
	...
	<div id="panel">
		<?php
		$sql="select * from re5 where cnum='".$data["b_idx"]."' order by cindex";
					...
					<table>
						<tr>
							...
							<td>
								...
								<input type="button" value="x" style="width:15px;height:15px;color:red;border:0px;background-color:#fff;-webkit-appearance:none; -moz-appearance:none; appearance:none;" onClick="delete_comment('<?=$com["cname"]?>', '<?=$com["cindex"]?>')"></font>
								<?php
								}
								?>
							</td>
...
<form method="post" name="formcomment" action="suwon_sellcomment_save.php"  style="background-color:#fff">
...
<table style="width:100%;height:11%;margin-top:-3%;margin-bottom:0%;background-color:#DCDDDD;">
    <tr>
		<td width="34%" align="center" style="padding-left:1%;">
			<a href="./suwon_sellboard_view.php?b_num=<?=$data["b_num"]+1?>">
        ...
        <input style="width:55%;height:86%;background-color:#898989;color:#fff;border:1px solid #fff;padding-top:2px;padding-bottom:2px;-webkit-appearance:none;" type="button" value=" 목록 " onClick="location.href='./suwon_sellboard.php?page=<?=$_GET["page"]?>';">
        </td>
		<td width="33%">
		<?php
		...
		?>
        <input style="width:40px;background-color:#898989;color:#fff;border:none;padding:2px;border:1px solid #fff;padding-bottom:4px;" type="button" value="수정" onClick="location.href='./suwon_sellmodify.php?b_idx=<?=$data["b_idx"]?>';">
        <?php
		}
		?>
		</td>
...
function board_delete(b_idx)
{
    if(confirm('글을 삭제 하시겠습니까?'))
		location.href='./suwon_selldelete.php?b_idx=' + b_idx;
}
...
function delete_comment(cname, cindex)
{
	if(confirm("삭제 하시겠습니까?"))
		location.href='suwon_sellboard_delete_comment.php?cname='+cname+'&cindex='+cindex;
}
