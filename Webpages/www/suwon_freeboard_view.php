...
$insertq="update tb_board2 set view=view+1 where b_num='".$_GET['b_num']."' ";
$insertr= sql_query($insertq);
$insertb="update tb_board2 set best=best+1 where b_num='".$_GET['b_num']."' ";
...
<!-------------------------------------------->	
<?
$sql = "select * from tb_board2 where b_idx = '".$_GET["b_num"]."' ";
...
<form method="post" name="formcomment" action="suwon_freecomment_save.php"  style="background-color:#fff">
<table style="width:100%;margin:0;">
			<tr>
			<td style="width:80%;height:33px">
				<input name="b_comment" id="cs" value=" 댓글을 입력하세요" onfocus="this.value=''" style="width:95%;height:100%;font-size:90%;margin-left:5%;padding-left:2%;-webkit-appearance:none;background-color:#EFEFEF;font-family: 'Nanum Gothic', serif;" size="35" autocomplete="off"></input>
			</td>
...
<table style="width:100%;height:11%;margin-top:-3%;margin-bottom:0%;background-color:#DCDDDD;">
    <tr>
		...
		?>
        <input style="width:40px;background-color:#898989;color:#fff;border:none;padding:2px;border:1px solid #fff;padding-bottom:4px;" type="button" value="수정" onClick="location.href='./suwon_freemodify.php?b_idx=<?=$data["b_idx"]?>';">
        <?php
		}
		?>
		</td>
...
function board_delete(b_idx)
{
    if(confirm('글을 삭제 하시겠습니까?'))
		location.href='./suwon_freedelete.php?b_idx=' + b_idx;
}
