	...
	<div class="board">
		<div class="boardhead">
			<div style="width:49%;height:20px;float:left">
				<table style="color:#fff;line-height:140%;"><tr><td><b>&nbsp;&nbsp;익명소통공간</b></td></tr></table>
			</div>
			<div style="width:49%;height:20px;float:left">
				<a href="./suwon_freewrite.php">
				<table style="">
					<tr>
...
$page_scale = 3;
$paging_str = "";
$sql = "select count(*) as cnt from tb_board2";
$total_count = sql_total($sql);
$paging_str = paging($page, $page_row, $page_scale, $total_count);
$from_record = ($page - 1) * $page_row;

$query = "select * from tb_board2 order by b_num desc, b_reply asc limit ".$from_record.", ".$page_row;
...
?>
	...
	<tr>
		<td  style="width:87%;align:left;background-color:#fff;padding:3px;padding-left:7px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;border-bottom:1px solid #ccc;">
			<!--inner table-->
			<table style="width:100%;height:100%;table-layout: fixed;" onClick="location.href='./suwon_freeboard_view.php?b_num=<?=$data["b_num"]?>&page=<?=$page?>'">
			<tr>
		...
		<td style="width:13%;padding-right:4%;colspan:2;border-bottom:1px solid #ccc;" onClick="location.href='./suwon_freeboard_view.php?b_num=<?=$data["b_num"]?>&page=<?=$page?>'">
				<?php
				if($cresult[0]>0)
				{
				?>
