...
$insertq="update tb_board4 set view=view+1 where b_num='".$_GET['b_num']."' ";
$insertr= sql_query($insertq);
$insertb="update tb_board4 set best=best+1 where b_num='".$_GET['b_num']."' ";
...
<!-------------------------------------------->	
<?
$sql = "select * from tb_board4 where b_idx = '".$_GET["b_num"]."' ";
...
<form method="post" name="formcomment" action="suwon_classcomment_save.php"  style="background-color:#fff">
...
<table style="width:100%;height:11%;margin-top:-3%;margin-bottom:0%;background-color:#DCDDDD;">
    <tr>
		<td width="34%" align="center" style="padding-left:1%;">
			<a href="./suwon_classboard_view.php?b_num=<?=$data["b_num"]+1?>">
        ...
        <input style="width:55%;height:86%;background-color:#898989;color:#fff;border:1px solid #fff;padding-top:2px;padding-bottom:2px;-webkit-appearance:none;" type="button" value=" 목록 " onClick="location.href='./suwon_classboard.php?page=<?=$_GET["page"]?>';">
        </td>
		<td width="33%">
		<?php
		...
		?>
        <input style="width:40px;background-color:#898989;color:#fff;border:none;padding:2px;border:1px solid #fff;padding-bottom:4px;" type="button" value="수정" onClick="location.href='./suwon_classmodify.php?b_idx=<?=$data["b_idx"]?>';">
        <?php
		}
		?>
		</td>
...
function board_delete(b_idx)
{
    if(confirm('글을 삭제 하시겠습니까?'))
		location.href='./suwon_classdelete.php?b_idx=' + b_idx;
}
...
function delete_comment(cname, cindex)
{
	if(confirm("삭제 하시겠습니까?"))
		location.href='suwon_classboard_delete_comment.php?cname='+cname+'&cindex='+cindex;
}
